    ...
    <section class="museum-detail">
      <v-container>
        ...
        <div class="text">
          <v-container>
            <client-only>
              <p v-html="museumN?.data.attributes.body"></p>
            </client-only>
          </v-container>
        </div>
      </v-container>
...
const { data: museumN, refresh: refreshPrices } = await useAsyncData(
  "lamps",
  () => findOne<any>("lamps", route.params.nahladSlug),
  {
    locale: locale.value,
  }
);
...
useHead({
  title: museumN.value?.data.attributes.title,
...
const thumbImage = computed(() => {
  return (museumN.value?.data.attributes.rotateGallery.data || []).map(
    (image: any) => '"' + store.getMediaUrl(image.attributes.formats.large.url) + '"'
  );
});

const galleryImages = computed(() => {
  return (museumN.value?.data.attributes.gallery.data || []).map((obj: any) => {
    /* console.log(obj); */
    /* return obj.id; */
    return {
      id: obj.id,
...
onMounted(async () => {
  store.setTitle(museumN.value?.data.attributes.title);
